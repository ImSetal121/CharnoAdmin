...
使用 shadcn/ui 的 `Pagination` 组件，并包含每页条目数量选择器：
...
**布局说明：**
- 使用三列网格布局（`grid grid-cols-3`）实现左右平衡
- 第一列：每页显示选择器和总记录数（居左，使用 `whitespace-nowrap` 防止文字换行）
- 第二列：分页导航（居中，使用 `flex justify-center`）
- 第三列：空占位（用于平衡布局）
